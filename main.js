...
d3.csv("weather.csv").then((data) => {
  ...
  /*
    ...
    */
  const filteredData1 = data; // My code here!
  ...
  const groupedData2 = d3
    .groups(filteredData2, (d) => d.month)
    .map(([month, entries]) => ({
      month,
  ...
  /*
    ...
    */
  const pivotedData = groupedData2.flatMap(
    ({ month, actualPrecip, avgPrecip, recordPrecip }) => [
      { month, precipitation: actualPrecip, type: "Actual" },
      { month, precipitation: avgPrecip, type: "Average" },
      { month, precipitation: recordPrecip, type: "Record" },
    ]
  );
